...
public class Lab0 {
	...
	public static double windChill(double airTemp, double windSpeed) {
		...
		double windTemp = A + (B * airTemp) + ((C * airTemp) - (D)) * (Math.pow(windSpeed, E));
		return windTemp;
	}
	...
	public static boolean isOnUnitCircle(double x, double y) {
		...
		double checkXY = checkX + checkY;

		boolean result = (checkXY == 1.00);
	...
	public static boolean isInsideUnitSquare(double x, double y) {
		boolean checkX = x <= 1.00 && x >= 0.00;
		boolean checkY = y <= 1.00 && y >= 0.00;

		boolean checkXY = (checkX == true && checkY == true);
	...
	public static boolean isOutsideUnitSquare(double x, double y) {
		...
		boolean checkX = x < 0.00 || x > 1.00;
		boolean checkY = y < 0.00 || y > 1.00;
	...
	public static char middleChar(String s) {
		...
		int length = s.length();
		int lengthIndex = 0;
		int lengthTest = length % 2;
		int halfLength = length / 2;

		lengthIndex = halfLength;

		return s.charAt(lengthIndex);
